...
resourcename="test1"  # lower case, only
resourcegroup="test1_rg"
...
# git clone https://github.com/apilogicserver/test1.git
# cd test1
...
az group create --name test1_rg --location "westus"
az appservice plan create --name myAppServicePlan --resource-group test1_rg --sku S1 --is-linux
az container create --resource-group test1_rg --name test1 --image apilogicserver/test1:latest --dns-name-label test1 --ports 5656 --environment-variables 'VERBOSE'='True'  'APILOGICPROJECT_CLIENT_URI'='//test1.westus.azurecontainer.io:5656'
